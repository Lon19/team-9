...
CREATE TABLE IF NOT EXISTS `codeforgoodteam9`.`universities` (
  ...
  INDEX `country_of_study_idx` (`country` ASC),
  CONSTRAINT `country_of_study`
    FOREIGN KEY (`country`)
    REFERENCES `codeforgoodteam9`.`users` (`country_of_study`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_general_ci
COMMENT = '		';
...
CREATE TABLE IF NOT EXISTS `codeforgoodteam9`.`courses` (
  ...
  INDEX `university_idx` (`university_id` ASC),
  CONSTRAINT `university`
    FOREIGN KEY (`university_id`)
    REFERENCES `codeforgoodteam9`.`universities` (`university_id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
DELIMITER $$
USE `codeforgoodteam9`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_course`(in Course_id VARCHAR(45), Name VARCHAR(45), Entry_requirements VARCHAR(45))
BEGIN
	insert into codeforgoodteam9.courses(course_id, name, entry_requirements)
